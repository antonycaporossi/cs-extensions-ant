...
//        mainUrl to "Top 10 Serie TV",
        ...
        "$mainUrl/serie-tv-generi/italiane/" to "Serie TV: Italiane",
        "$mainUrl/serie-tv-generi/turche/" to "Serie TV: Turche",
        //"$mainUrl/film-generi/italiani/" to "Film: Italiani",
    ...
    private suspend fun posterInterceptorDirtyFix(url: String): Unit {
        val loadan = app.get(url, headers = interceptor.getCookieHeaders(url).toMap()).document
    }
        ...
        val searchResponses = when (section) {
            "Film: Ultimi aggiunti", "Serie TV: Ultime aggiunte" -> {
                ...
                items.map {
                    ...
                    // posterInterceptorDirtyFix(url)
                    newTvSeriesSearchResponse(title, url) {
                        this.posterUrl = poster
                        this.posterHeaders = interceptor.getCookieHeaders(poster).toMap()
                    }
                }
            }
